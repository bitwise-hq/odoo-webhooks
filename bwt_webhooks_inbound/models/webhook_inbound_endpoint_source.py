...
class WebhookInboundEndpointSource(models.Model):
    _name = "bwt.webhook.inbound.endpoint.source"
    ...
    field_name = fields.Char(
        ...
        index=True,
        string="Resolved Key",
        help=("Free-form key for the resolved value. Use Semantic Bindings on the endpoint to connect built-in webhook semantics such as signature, event type, or delivery identity to one of these resolved keys. Any other key is allowed and will be stored with the inbound event."),
    )
    ...
    header_param_name = fields.Char(
        ...
        help="Parameter name inside a structured header such as Stripe-Signature or Paddle-Signature.",
    )
    ...
    normalize_mode = fields.Selection(

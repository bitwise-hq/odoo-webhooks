...
class WebhookInboundEvent(models.Model):
    ...
    def _compute_queue_job_identity_key(self):
        for event in self:
            event.queue_job_identity_key = (
                event._get_queue_job_identity_key() if event.id else False
            )
    ...
    def _prepare_create_values_from_request(
        ...
        *,
        ...
        return {
            ...
            "replayed_from_event_id": replayed_from_event.id
    ...
    def _log_rejected_request(
        ...
        *,
        ...
        resolved_values = (
            endpoint._extract_resolved_values(body, headers, payload_context)
            if endpoint
            else {}
        )
        metadata = (
            endpoint._extract_inbound_metadata(
                body, headers, payload_context, resolved_values=resolved_values
            )
            if endpoint
            else {}
        )
        ...
        if endpoint:
            try:
                delivery_identity_key, delivery_identity_source = (
                    endpoint._resolve_delivery_identity(body_sha256, metadata)
                )
            except ValidationError:
                delivery_identity_key = False
                delivery_identity_source = False
            try:
                replay_identity_key, replay_identity_source = (
    ...
    def _receive_webhook_request(self, endpoint, body, headers):
        ...
        try:
            payload = self._parse_payload(
                body.decode("utf-8", errors="replace"), raise_on_invalid=True
            )
            ...
            metadata = endpoint._extract_inbound_metadata(
                body, headers, payload, resolved_values=resolved_values
            )
    ...
    def _queue_processing(self):
        ...
        for event in self:
            ...
            event.with_user(runtime_user_id).with_delay(
                identity_key=event._get_queue_job_identity_key()
            ).process_event()
    ...
    def _execute_model_driven_handler(self, handler):
        self.ensure_one()
        rules = handler.inbound_rule_ids.filtered("active").sorted(
            key=lambda record: (record.sequence, record.id)
        )
        matched_rule = False
        for rule in rules:
            if all(
                self._inbound_condition_matches(condition)
        ...
        try:
            ...
            if matched_rule.action_type in (
                "create_record",
                "update_record",
                "upsert_record",
            ):
                model = self.env[matched_rule.target_model_name]
                values = self._build_inbound_assignment_values(
                    matched_rule, target_kind="field"
                )
                target_record = False
            ...
            if matched_rule.action_type == "queue_outbound":
                ...
                context_values = self._build_inbound_assignment_values(
                    matched_rule, target_kind="context_key"
                )
                    ...
                    self.env["webhook.outbound.delivery.context.line"].create(
                        {
                            ...
                            "key_name": key_name,
                            "source_kind": "literal",
                            "literal_value": json.dumps(value)
    ...
    def _resolve_inbound_source_value(
        self, source_kind, source_expression=False, literal_value=False
    ):
        self.ensure_one()
    ...
    def _inbound_condition_matches(self, condition):
        actual_value = self._resolve_inbound_source_value(
            condition.source_kind, condition.source_expression
        )
    ...
    def _build_inbound_lookup_domain(self, rule):
        self.ensure_one()
        domain = []
        for lookup in rule.lookup_ids.sorted(
            key=lambda record: (record.sequence, record.id)
        ):
            value = self._resolve_inbound_source_value(
                lookup.source_kind, lookup.source_expression, lookup.literal_value
            )
    ...
    def _build_inbound_assignment_values(self, rule, *, target_kind):
        self.ensure_one()
        values = {}
        for assignment in rule.assignment_ids.filtered(
            lambda record: record.target_kind == target_kind

...
class WebhookOutboundDelivery(models.Model):
    ...
    http_method = fields.Selection(
        related="endpoint_id.http_method", string="HTTP Method", readonly=True
    )
    target_url = fields.Char(
        related="endpoint_id.target_url", string="Target URL", readonly=True
    )
    timeout_seconds = fields.Integer(
        related="endpoint_id.timeout_seconds", readonly=True
    )
    ...
    def _compute_queue_job_identity_key(self):
        for delivery in self:
            delivery.queue_job_identity_key = (
                delivery._get_queue_job_identity_key() if delivery.id else False
            )
    ...
    def _get_next_replay_number(self):
        self.ensure_one()
        return (
            self.env["webhook.outbound.delivery"].search_count(
                [
    ...
    def _prepare_endpoint_snapshot_vals(self, endpoint, vals):
        ...
        prepared_vals.setdefault(
            "name", "%s / %s" % (endpoint.display_name, fields.Datetime.now())
        )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if endpoint_id:
                ...
                prepared_vals = self._prepare_endpoint_snapshot_vals(
                    endpoint, prepared_vals
                )
            prepared_vals["request_headers_json"] = self._normalize_headers_json(
                prepared_vals.get("request_headers_json")
            )
            prepared_vals["payload_json"] = self._normalize_payload_json(
                prepared_vals.get("payload_json")
            )
    ...
    def _get_context_values(self):
        self.ensure_one()
        values = {}
        for line in self.context_line_ids.filtered("active").sorted(
    ...
    def _build_request_data(self):
        ...
        headers = {}
        payload = {}
        for rule in self.endpoint_id.header_rule_ids.filtered("active").sorted(
            key=lambda record: (record.sequence, record.id)
        ):
            ...
            headers[rule.header_name] = (
                "" if value is False or value is None else str(value)
            )
        for rule in self.endpoint_id.payload_rule_ids.filtered("active").sorted(
            key=lambda record: (record.sequence, record.id)
        ):
            value = self._resolve_source_value(
                rule.source_kind,
    ...
    def _get_next_attempt_number(self):
        self.ensure_one()
        return (
            self.env["webhook.outbound.delivery.attempt"].search_count(
                [
                    ("delivery_id", "=", self.id),
                ]
            )
    ...
    def _create_attempt(
        ...
        *,
        ...
        values = {
            "name": "%s / Attempt %s"
            % (self.display_name, self._get_next_attempt_number()),
            "delivery_id": self.id,
            ...
            "response_headers_json": self._serialize_headers(response_headers or {})
            if response_headers
            else False,
    ...
    def _queue_processing(self):
        ...
        for delivery in self:
            ...
            delivery.with_user(runtime_user_id).with_delay(
                identity_key=delivery._get_queue_job_identity_key()
            ).process_delivery()
    ...
    def action_create_replay_delivery(self):
        ...
        replay_delivery = self.copy(
            {
                "name": "%s / Replay %s"
                % (self.display_name, self._get_next_replay_number()),
                "state": "draft",
                "queued_at": False,
                "processed_at": False,
                "processing_note": False,
        ...
        action["views"] = [
            (self.env.ref("webhooks.view_webhook_outbound_delivery_form").id, "form")
        ]
        return action
    ...
    def _execute_model_driven_handler(self, handler, request_data=None):
        ...
        context_values = self._get_context_values()
        rules = handler.outbound_rule_ids.filtered("active").sorted(
            key=lambda record: (record.sequence, record.id)
        )
        matched_rule = False
        for rule in rules:
            if all(
                self._outbound_condition_matches(
                    condition, context_values, request_data
                )
                for condition in rule.condition_ids.sorted(
                    key=lambda record: (record.sequence, record.id)
                )
            ):
                matched_rule = rule
        ...
        updated_request_data["payload"] = json.loads(
            json.dumps(request_data["payload"], default=json_default)
        )
        for assignment in matched_rule.assignment_ids.sorted(
            key=lambda record: (record.sequence, record.id)
        ):
            value = self._resolve_source_value(
            ...
            if assignment.target_scope == "request":
                ...
                updated_request_data["headers"][assignment.target_expression] = (
                    "" if value is False or value is None else str(value)
                )
            else:
                updated_request_data["payload"] = self._set_payload_path(
                    updated_request_data["payload"], assignment.target_expression, value
                )
        ...
        for delivery in self:
            ...
            try:
                ...
                if delivery.handler_id:
                    result = delivery.handler_id.execute_outbound(
                        delivery, request_data=request_data
                    )
                    if isinstance(result, dict):
                        matched_rule_id = result.get("matched_rule_id") or False
                    result, request_data = delivery._apply_handler_result(
                        result, request_data
                    )
                    if isinstance(result, dict):
                        ...
                        if status == "cancel":
                            delivery._create_attempt(
                                request_data, state="canceled", note=handler_note
                            )
                            delivery.write(
                                {
                                    "state": "canceled",
                        ...
                        if status == "dead_letter":
                            delivery._create_attempt(
                                request_data, state="dead_letter", note=handler_note
                            )
                            delivery.write(
                                {
                                    "state": "dead_letter",
                ...
                delivery.write(
                    {
                        "request_headers_json": delivery._serialize_headers(
                            request_data["headers"]
                        ),
                        "payload_json": delivery._serialize_payload(
            ...
            common_values = {
                ...
                "response_headers_json": json.dumps(
                    dict(response.headers.items()), indent=2, sort_keys=True
                ),
                ...
                attempt.write(
                    {
                        ...
                        "processing_error": False,
                        "response_status_code": response.status_code,
                        "response_headers_json": delivery._serialize_headers(

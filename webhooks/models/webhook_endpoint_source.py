...
class WebhookEndpointSource(models.Model):
    ...
    payload_path = fields.Char(
        help="Dotted path such as data.object.id or items.0.sku."
    )
    literal_value = fields.Char()
    computed_method = fields.Char(
        string="Compute Method", help="Endpoint method name used for computed sources."
    )
    ...
    hash_algorithm = fields.Selection(
        selection=_HASH_SELECTION, required=True, default="none", string="Hash"
    )

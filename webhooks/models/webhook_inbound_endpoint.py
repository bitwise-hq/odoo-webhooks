...
class WebhookInboundEndpoint(models.Model):
    _name = "webhook.inbound.endpoint"
    ...
    inbound_event_ids = fields.One2many(
        "webhook.inbound.event", "endpoint_id", string="Inbound Events"
    )
    ...
    def _compute_route_path(self):
        for endpoint in self:
            endpoint.route_path = (
                f"/webhooks/in/{endpoint.path}" if endpoint.path else False
            )
    ...
    def _check_signature_configuration(self):
        for endpoint in self:
            ...
            if (
                ...
                or endpoint.signature_max_future_skew_seconds > 0
            ):
    ...
    def _has_active_source_for_key(self, value_key):
        ...
        return bool(
            self.source_ids.filtered(
                lambda line: line.active and line.field_name == value_key
            )
        )

    def _extract_field_candidates(
        self, field_name, body, headers, payload, *, allow_multiple=False
    ):
        self.ensure_one()
        candidates = []
        ...
        for candidate_sequence in candidate_sequences:
            ...
            for line in lines:
                ...
                if isinstance(raw_value, list):
                    transformed = [
                        item
                        for item in line._apply_transforms(raw_value)
    ...
    def _extract_resolved_values(self, body, headers, payload):
        ...
        for field_name in self._get_configured_field_names():
            resolved_values[field_name] = self._extract_field_value(
                field_name, body, headers, payload
            )
    ...
    def _extract_semantic_value(
        self, semantic_name, body, headers, payload, *, resolved_values=None
    ):
        self.ensure_one()
        value_key = self._get_bound_value_key(semantic_name)
        if not value_key:
        ...
        candidates = self._extract_semantic_candidates(
            semantic_name, body, headers, payload
        )
        return candidates[0] if candidates else False

    def _extract_inbound_metadata(
        self, body, headers, payload, *, resolved_values=None
    ):
        self.ensure_one()
    ...
    def _get_signature_secrets(self):
        ...
        return [
            secret
            for secret in [self.signature_secret, self.signature_secondary_secret]
            if secret
        ]

    def _build_signature_message(self, body, headers, payload):
        self.ensure_one()
        parts = self.signature_part_ids.filtered("active").sorted(
            key=lambda part: (part.sequence, part.id)
        )
        if not parts:
    ...
    def _validate_signature_freshness(self, raw_timestamp):
        self.ensure_one()
        if (
            self.signature_max_age_seconds <= 0
            and self.signature_max_future_skew_seconds <= 0
        ):
            return
    ...
    def _verify_signature(self, body, headers, payload, metadata):
        ...
        if self.signature_verification_mode != "hmac":
            return
        incoming_values = self._extract_semantic_candidates(
        ...
        for secret in secrets:
            ...
            for candidate in cleaned_values:
                if hmac.compare_digest(candidate, expected):
                    self._validate_signature_freshness(
                        metadata.get("signature_timestamp")
                    )
                    return
        raise WebhookSignatureValidationError(

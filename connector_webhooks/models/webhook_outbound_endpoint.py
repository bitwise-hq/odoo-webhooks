...
class WebhookOutboundEndpoint(models.Model):
    _inherit = "webhook.outbound.endpoint"

    _connector_backend_ref_uniq = models.Constraint(
        "unique(connector_backend_ref)",
        "A connector backend can only be linked to one outbound webhook endpoint.",
    )
        ...
        for endpoint in self:
            ...
            if backend_company and backend_company != endpoint.company_id:
                raise ValidationError(
                    self.env._(
                        "The linked connector backend company must match the outbound endpoint company."
                    )
                )

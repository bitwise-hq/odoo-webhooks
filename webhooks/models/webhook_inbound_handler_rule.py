...
class WebhookHandlerInboundRule(models.Model):
    ...
    condition_ids = fields.One2many(
        "webhook.handler.inbound.rule.condition", "rule_id", string="Conditions"
    )
    ...
    execution_ids = fields.One2many(
        "webhook.inbound.rule.execution", "rule_id", string="Executions"
    )

    @api.constrains(
        "action_type", "retry_seconds", "target_model_name", "outbound_endpoint_id"
    )

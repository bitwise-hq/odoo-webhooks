...
class TestWebhookInboundEventRequest(WebhookInboundEventTestCase):
    def test_payload_resolution_and_lookup(self):
    ...
    def test_rejection_logging_and_queue_observability(self):
        endpoint = self._create_inbound_endpoint()
        event_model = self.env["webhook.inbound.event"]

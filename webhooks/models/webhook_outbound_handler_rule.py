...
class WebhookHandlerOutboundRule(models.Model):
    ...
    result_status = fields.Selection(
        selection=OUTBOUND_RULE_STATUS_SELECTION, required=True, default="send"
    )
    note = fields.Text()
    retry_seconds = fields.Integer(default=0)
    condition_ids = fields.One2many(
        "webhook.handler.outbound.rule.condition", "rule_id", string="Conditions"
    )
    assignment_ids = fields.One2many(
        "webhook.handler.outbound.assignment", "rule_id", string="Assignments"
    )

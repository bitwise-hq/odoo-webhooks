...
class WebhookRuleTestCase(TransactionCase):
    def setUp(self):
        ...
        self._token_index = 0
        self.execution_user = self.env['res.users'].with_context(no_reset_password=True).create({
    ...
    def _create_inbound_endpoint(self, *, handler=False, **values):
        ...
        create_vals = {
            ...
            'delivery_identity_policy': values.pop('delivery_identity_policy', 'body_sha256'),
    ...
    def _create_inbound_event(self, endpoint, *, handler=False, resolved_values=None, payload=None, headers=None, **values):
        ...
        create_vals = {
            ...
            'execution_user_id': values.pop('execution_user_id', endpoint.execution_user_id.id),
            'handler_id': values.pop('handler_id', handler.id if handler else endpoint.handler_id.id),
            ...
            'delivery_identity_source': values.pop('delivery_identity_source', 'delivery_id'),
            ...
            'resolved_values_json': values.pop('resolved_values_json', self._serialize(resolved_values or {})),
            'body_sha256': values.pop('body_sha256', body_sha256),
            'request_body': request_body,
            'request_headers_json': values.pop('request_headers_json', self._serialize(headers_value)),

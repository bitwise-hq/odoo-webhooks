...
class TestOutboundModelDrivenRules(WebhookRuleTestCase):
    def test_build_request_data_uses_endpoint_rules_and_context_lines(self):
        ...
        self._create_outbound_context_line(
            delivery, key_name="trace_id", literal_value="trace-001"
        )
        self._create_outbound_context_line(
            delivery, key_name="customer_name", literal_value="Alice Example"
        )
        self._create_outbound_context_line(
            delivery, key_name="amount", literal_value="149.99"
        )
    ...
    def test_handler_rule_mutates_request_and_requests_retry(self):
        ...
        self.assertEqual(
            updated_request_data["target_url"], "https://override.example.com/hook"
        )
    ...
    def test_delivery_uses_current_endpoint_target_url(self):
        endpoint = self._create_outbound_endpoint(
            target_hostname="https://example.com", target_path="/original"
        )
        delivery = self._create_outbound_delivery(endpoint)
        ...
        self.assertEqual(
            delivery.target_url, "https://override.example.com/hooks/orders"
        )
        self.assertEqual(
            request_data["target_url"], "https://override.example.com/hooks/orders"
        )
    ...
    def test_build_request_data_normalizes_datetime_payload_values(self):
        ...
        self.assertIn(
            fields.Datetime.to_string(delivery.create_date),
            delivery._serialize_payload(request_data["payload"]),
        )
    ...
    def test_outbound_endpoint_state_actions_reject_invalid_states(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, "Only active or archived outbound endpoints"
        ):
            draft_endpoint.action_set_draft()
        with self.assertRaisesRegex(
            ValidationError, "Only draft or active outbound endpoints"
        ):
    ...
    def test_reset_to_draft_rejects_non_resettable_states(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, "Only failed, dead-letter, or canceled"
        ):
